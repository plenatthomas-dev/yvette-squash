...
//      « + », et c'est le piège que ce module existe pour éviter — l'écran ne trace plus que le
//      RANG, `mean` s'étant révélé être la même grandeur (r = 1,000) ;
//   4. le filtrage est LOCAL — cocher un joueur ne redemande rien au serveur ;
//   5. les MARCHES DU CLASSEMENT ne bougent pas quand la sélection change — un repère qui suit
//      ce qu'on regarde n'est plus un repère — et les zones qu'elles bornent sont peintes en
//      UNE teinte dosée, jamais une couleur par échelon.
...
function serie(
  id: string,
  name: string,
  vals: Array<{ rangM: number | null } | null>,
): HistorySeries {
  return {
    id,
    kind: "member",
    ...
    points: vals.flatMap((v, i) =>
      v === null ? [] : [{ month: MOIS[i], clt: "5A", rang: null, rangM: v.rangM, mean: null }],
    ),
  };
}
...
function serieCltee(
  id: string,
  name: string,
  vals: Array<{ clt: string; rangM: number } | null>,
): HistorySeries {
  return {
    id,
    kind: "member",
    ...
    team: null,
    points: vals.flatMap((v, i) =>
      v === null ? [] : [{ month: MOIS[i], clt: v.clt, rang: null, rangM: v.rangM, mean: null }],
    ),
  };
}
...
describe("RankingHistory", () => {
  it("ouvre sur SA propre courbe quand on reconnaît le joueur connecté", async () => {
    monte(
      [
        serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 1800 }]),
        serie("u2", "Paul Martin", [{ rangM: 2600 }, null, null]),
      ],
      "Jean Dupont",
    );
    await souffle();
  ...
  it("à défaut de se reconnaître, ouvre sur les joueurs les mieux mesurés plutôt que sur du vide", async () => {
    monte([
      serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, null]),
  ...
  it("la légende porte la dernière valeur ET l'évolution, en toutes lettres", async () => {
    monte([serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 1800 }])], "Jean Dupont");
    await souffle();
    const li = legende("Jean Dupont");
    expect(li).not.toBeNull();
    expect(within(li as HTMLElement).getByText("#1800")).toBeTruthy();
    expect(within(li as HTMLElement).getByText("+500")).toBeTruthy();
  });
  ...
  it("descendre au classement s'affiche comme un progrès", async () => {
    monte([serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 1800 }])], "Jean Dupont");
    await souffle();
  ...
  it("cocher un joueur trace sa courbe SANS rien redemander au serveur", async () => {
    monte(
      [
        serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 1800 }]),
        serie("u2", "Paul Martin", [{ rangM: 2600 }, null, { rangM: 2500 }]),
      ],
      "Jean Dupont",
    );
    await souffle();
  ...
  it("resserrer la plage de mois retire les mesures hors plage", async () => {
    ...
    await souffle();
    expect(within(legende("Jean Dupont") as HTMLElement).getByText("+500")).toBeTruthy();
    // On s'arrête en février : la dernière mesure visible devient le 2100e.
    fireEvent.change(screen.getByLabelText("Jusqu'au mois"), { target: { value: "2026-02-02" } });
    const li = legende("Jean Dupont") as HTMLElement;
    expect(within(li).getByText("#2100")).toBeTruthy();
    expect(within(li).getByText("+200")).toBeTruthy();
  });

  it("une mesure identique d'un bout à l'autre affiche « ±0 », jamais « −0 »", async () => {
    // « −0 » se lit comme une baisse, et n'en est pas une.
    monte([serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 2300 }])], "Jean Dupont");
    await souffle();
  ...
  it("tout décocher n'affiche pas un graphique vide mais dit quoi faire", async () => {
    monte([serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, null])], "Jean Dupont");
    await souffle();
  ...
  it("dit lui-même que l'historique n'a pas été rempli quand la plupart n'ont qu'une mesure", async () => {
    monte(
      [
        serie("u1", "Jean Dupont", [{ rangM: 2300 }, { rangM: 2200 }, { rangM: 1800 }]),
        serie("u2", "Paul Martin", [null, null, { rangM: 2600 }]),
        serie("u3", "Luc Bernard", [null, null, { rangM: 2500 }]),
      ],
      "Jean Dupont",
    );
    await souffle();
  ...
  it("se tait quand l'historique est correctement rempli", async () => {
    monte(
      [
        serie("u1", "Jean Dupont", [{ rangM: 2300 }, { rangM: 2200 }, null]),
        serie("u2", "Paul Martin", [{ rangM: 2600 }, { rangM: 2550 }, null]),
...
describe("les graduations de l'axe", () => {
  it("n'affiche pas un rang fractionnaire au milieu de l'échelle", async () => {
    ...
    monte(
      [serie("u1", "Jean Dupont", [{ rangM: 1800 }, null, { rangM: 2301 }])],
      "Jean Dupont",
    );
    await souffle();
  ...
  it("n'offre plus de choix de métrique : les deux disaient la même chose", async () => {
    // `mean` et `rangM` valent r = 1,000 l'un pour l'autre. Le sélecteur proposait donc un
    // choix sans conséquence — et sa branche « Points » était en plus dessinée à l'envers.
    monte([serie("u1", "Jean Dupont", [{ rangM: 2300 }, null, { rangM: 1800 }])], "Jean Dupont");
    await souffle();
    expect(screen.queryByRole("button", { name: "Rang" })).toBeNull();
    expect(screen.queryByRole("button", { name: "Points" })).toBeNull();
    expect(document.querySelector(".rankhist-graph")).toBeTruthy();
  });

  it("peint les zones de classement DERRIÈRE les courbes, en une rampe ordonnée", async () => {
    monte(
      [
        serieCltee("u1", "Jean Dupont", [
          { clt: "5B", rangM: 2400 },
    ...
    await souffle();
    const bandes = [...document.querySelectorAll(".rankhist-bande")];
    expect(bandes.length).toBeGreaterThanOrEqual(2);

    // UNE RAMPE, PAS UNE COULEUR PAR CATÉGORIE : une seule teinte, dosée en opacité. Si un jour
    // quelqu'un remplace ça par une couleur par échelon, l'ordre du graphique se perd et ce
    // test tombe.
    const remplissages = new Set(bandes.map((b) => getComputedStyle(b).fill));
    expect(remplissages.size).toBe(1);
    const opacites = bandes.map((b) => Number(b.getAttribute("fill-opacity")));
    expect(new Set(opacites).size).toBe(opacites.length);

    // Et derrière tout : la première bande précède la première courbe dans le document.
    const svg = document.querySelector(".rankhist-graph") as SVGElement;
    const noeuds = [...svg.querySelectorAll(".rankhist-bande, .rankhist-trace")];
    expect(noeuds[0].classList.contains("rankhist-bande")).toBe(true);
    expect(noeuds[noeuds.length - 1].classList.contains("rankhist-trace")).toBe(true);
  });
  ...
  it("place la marche sur TOUT le corpus, pas sur les seules courbes tracées", async () => {
    ...
    // mesure RESSERRE l'encadrement de la frontière (un rang PLUS PETIT est un MEILLEUR
    // classement, donc 5A est au-dessus de 5B) :
    //   — sur le corpus  : min(5B) = 2300 (Marie), max(5A) = 1900 (Jean) → ligne à 2100 ;
    //   — sur Jean seul  : min(5B) = 2400,         max(5A) = 1900        → ligne à 2150.
    // Les deux tombent dans la fenêtre visible (1900–2400), donc seule l'ordonnée les sépare :
    ...
    monte(
      [
        ...
        serieCltee("u2", "Marie Martin", [{ clt: "5B", rangM: 2300 }, null, null]),
      ],
      "Jean Dupont",
    );
    await souffle();

    // Cadre : h 170, padT 10, padB 22 → 138 px utiles, axe INVERSÉ (le petit rang en haut).
    const y = (v: number) => 10 + 138 * ((v - 1900) / 500);
    const trace = Number(document.querySelector(".rankhist-palier")?.getAttribute("y1"));
    expect(trace).toBeCloseTo(y(2100), 1);
    expect(trace).not.toBeCloseTo(y(2150), 1);
  });
